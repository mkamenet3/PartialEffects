...
makeplot1 <- function(dat, x, filterstring, mse=TRUE, titlestring, xlabstring, ylabstring){
  if(mse==TRUE){
    dat %>%
      ...
      ggplot() +
      geom_line(aes(y=log(mean_mse), x=.data[[x]], group=name.x, color=name.x), size=2) +
      geom_point(aes(y=log(mean_mse), x=.data[[x]], shape=name.x), size=3) +
...
(p_samplesize_mse <- makeplot1(dat=mainB,x="N", filterstring = c("base0","base0 bigN", "base0 biggerN", "base0 smallN"),
          ...
          ylabstring = "(Log) MSE (higher is worse)")) 
...
(Suggested by coauthors)

```{r, eval=FALSE}
test <- mainB%>%
      dplyr::filter(type %in% c("base0","base0 missingvars")) %>%
      pivot_longer(cols=c(`Positive Effect`, `Negative Effect`), names_to = "posneg", values_to = "mse")%>% 
      pivot_longer(cols=c(biaspos,biasneg), names_to = "posneg2", values_to = "bias") %>%
      group_by(type, name.x,N, jointpos, jointneg, Nexposures, Nexposures0, posneg, posneg2, corr, imbalance, missing) %>%
      dplyr::summarize(mean_mse = mean(mse),
      ...
      mutate(name.x = if_else( name.x2=="QGC: A Priori", "QGCAP",
                               if_else(name.x2 == "QGC: Enet", "QGCEN", name.x2))) 
      ggplot(data=test) +
      geom_line(aes(y=log(mean_mse), x=missing,group=name.x, color=name.x), size=2) 
      geom_point(aes(y=log(mean_mse), x=type, shape=name.x), size=3) +
      scale_shape_manual(values=1:9)+
      facet_grid(.~posneg) + theme_bw()
      ggtitle(titlestring) + xlab(xlabstring)+ylab(ylabstring) + theme(legend.position = "bottom",legend.title = element_blank())+
      scale_color_manual(values=mycols) 
```


```{r}
(p_missing_mse <- makeplot1(dat = mainB, x= "missing",filterstring = c("base0","base0 missingvars"),
          mse=TRUE, titlestring = "Missing positive exposure", 
          xlabstring = "", 
          ylabstring = "(Log) MSE (higher is worse)") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)))
  

(p_missing_bias <- makeplot1(dat = mainB, x= "missing",filterstring = c("base0","base0 missingvars"),
          mse=FALSE, titlestring = "Missing positive exposure", 
          xlabstring = "", 
          ylabstring = "Average Bias (zero is best)")+
          theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)))
...
p_corr_mse + p_samplesize_mse +  p_spread_mse + p_imbalance_mse + plot_layout(ncol=2) + plot_annotation(tag_levels = "A", title="Comparison of Mean Squared Error (MSE) Across Methods Estimating Partial Effects")
...
tab %>%
  ...
  dplyr::select(-name.x) %>%
  arrange(N, name) %>%
  mutate(`mean_mse_Negative Effect` = log(`mean_mse_Negative Effect`),
         `mean_mse_Positive Effect` = log(`mean_mse_Positive Effect`)) %>%
  gt() %>%
